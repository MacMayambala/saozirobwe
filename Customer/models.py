...
class Customer(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.cus_id:
            branch_code = self.branch.code.strip() if self.branch.code else '0'
            if len(branch_code) > 1:
                branch_code = branch_code[0]
            year_suffix = datetime.now().year % 100
            last_customer = Customer.objects.filter(cus_id__startswith=f"{branch_code}{year_suffix:02d}").order_by('-cus_id').first()
            if last_customer:
